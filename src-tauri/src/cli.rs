...
impl Cli
{
  pub fn parse_or_default() -> Self
  {
    let args =
    {
      ...
      {
        warn!("При запуске программы не обнаружены аргументы --host --ws_port и --api_port, будут использоваться агрументы для локального сервера -> {}", parsed.err().unwrap().to_string());
        Cli::default()
      }
    };
    args
